...
async function fda(params,base){
  ...
  try{
    ...
    if(cached && Date.now()-cached.ts<FDA_CACHE_TTL)return cached.data;
...
async function withBtn(id,fn,silent){
  ...
  const btn=$(id);const old=btn.textContent;btn.disabled=true;btn.textContent='Working…';
  try{await fn();trackToolUse();}finally{btn.disabled=false;btn.textContent=old;}
}
  ...
  await withBtn('drugBtn',async()=>{
    ...
    try{
      ...
      $('drugOut').innerHTML=`
        ...
        <div class="actions">
          <button class="btn ghost" onclick='exportDrugCSV(${esc(JSON.stringify(displayName))},${esc(JSON.stringify(rx))})'>Export CSV</button>
          <button class="btn ghost" onclick="go('outcomes');$('outDrug').value=${esc(JSON.stringify(displayName))};outcomes()">Outcome severity →</button>
        </div>`;
  ...
  await withBtn('rxBtn',async()=>{
    ...
    try{
      ...
      $('rxOut').innerHTML=`
        ...
        <div class="actions">
          <button class="btn ghost" onclick='exportReactionCSV(${esc(JSON.stringify(displayName))},${esc(JSON.stringify(drugs))})'>Export CSV</button>
          <button class="btn ghost" onclick="go('drug');$('drugName').value=${esc(JSON.stringify(top10[0]?.term||''))};drugSearch()" ${top10.length?'':'disabled'}>Analyze top drug →</button>
        </div>`;
  ...
  await withBtn('cmpBtn',async()=>{
    ...
    try{
      ...
      $('cmpOut').innerHTML=`
        ...
        <h4>Total reports</h4>
        <canvas id="cmpTotals" height="70"></canvas>
...
async function recent(silent){
  await withBtn('recentBtn',async()=>{
    ...
    try{
      ...
      $('recentOut').innerHTML=`<table><tr><th>ID</th><th>Received</th><th>Age</th><th>Sex</th><th>Drugs</th><th>Reactions</th></tr>${rows||'<tr><td colspan=6>No data.</td></tr>'}</table>
        <div class="actions"><button class="btn ghost" onclick='exportRecentCSV(${esc(JSON.stringify(out))})'>Export CSV</button></div>
        ${methodPanel([
          ['Source','FDA openFDA Drug Adverse Event API — data from the FDA Adverse Event Reporting System (FAERS), <code>/drug/event.json</code>'],
